    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className="px-3.5 py-2 rounded-xl border border-white/60 bg-white/50 hover:bg-white text-xs text-[#1a1a2e] hover:text-[#7c3aed] font-mono font-semibold flex items-center gap-1.5 transition-all shadow-xs cursor-pointer"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div 
          ...
          className="card p-5 rounded-2xl flex flex-col justify-between h-32 cursor-pointer transition-all group border-l-4 border-l-[#7c3aed] hover:border-r-4 hover:border-r-[#7c3aed]"
        >
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-[#64748b] uppercase tracking-wider">Current capital</span>
            <Edit2 className="w-4 h-4 text-[#64748b] group-hover:text-[#7c3aed] transition-colors" />
          </div>
          <div>
            <h3 className="text-2xl font-bold text-[#1a1a2e] group-hover:text-[#7c3aed] font-mono transition-colors">
              ₹{capital.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </h3>
            <span className="text-[11px] text-[#7c3aed] font-semibold block mt-1">Click to customize capital balance</span>
          </div>
        </div>

        {/* Card: Today's PnL */}
        <div className="card p-5 rounded-2xl flex flex-col justify-between h-32 border-l-4 border-l-[#10b981]">
          <span className="text-xs font-bold text-[#64748b] uppercase tracking-wider">Today&apos;s PnL</span>
          <div>
            <h3 className={`text-2xl font-bold font-mono ${isFreshMode ? 'text-[#1a1a2e]' : 'text-[#10b981]'}`}>
              {isFreshMode ? '₹0.00' : '+₹24,500.00'}
            </h3>
            <span className="text-[11px] text-[#64748b] font-medium block mt-1">{isFreshMode ? 'Fresh Session Baseline' : '+2.45% intraday ROI'}</span>
          </div>
        </div>

        {/* Card: Weekly PnL */}
        <div className="card p-5 rounded-2xl flex flex-col justify-between h-32 border-l-4 border-l-[#10b981]">
          <span className="text-xs font-bold text-[#64748b] uppercase tracking-wider">Weekly PnL</span>
          <div>
            <h3 className={`text-2xl font-bold font-mono ${isFreshMode ? 'text-[#1a1a2e]' : 'text-[#10b981]'}`}>
              {isFreshMode ? '₹0.00' : '+₹89,100.00'}
            </h3>
            <span className="text-[11px] text-[#64748b] font-medium block mt-1">{isFreshMode ? 'Fresh Session Baseline' : '+8.91% weekly ROI'}</span>
          </div>
        </div>

        {/* Card: Max Drawdown */}
        <div className="card p-5 rounded-2xl flex flex-col justify-between h-32 border-l-4 border-l-[#ef4444]">
          <span className="text-xs font-bold text-[#64748b] uppercase tracking-wider">Max Drawdown</span>
          <div>
            <h3 className={`text-2xl font-bold font-mono ${isFreshMode ? 'text-[#1a1a2e]' : 'text-[#ef4444]'}`}>
              {isFreshMode ? '0.00%' : '-4.12%'}
            </h3>
            <span className="text-[11px] text-[#64748b] font-medium block mt-1">Controlled peak-to-trough</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {[
          ...
          <div key={item.label} className="card p-4 rounded-2xl flex flex-col justify-between h-20">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card p-6 rounded-2xl">
            <div className="flex justify-between items-center mb-6">
              ...
              {isFreshMode && (
                <span className="text-xs font-mono text-[#7c3aed] font-semibold">Fresh Session Baseline</span>
              )}
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {[
                ...
                <div key={stat.label} className="border-b border-slate-100 pb-3">
                  <span className="text-xs text-[#64748b] font-medium block mb-1">{stat.label}</span>
                  <span className="text-sm font-bold text-[#1a1a2e] font-mono">{stat.val}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="card p-6 rounded-2xl">
            <div className="mb-6 flex justify-between items-center">
              ...
              <span className="text-xs text-[#64748b] font-medium">Dynamic Proportional Allocation</span>
            </div>
                ...
                return (
                  <div key={asset.name} className="space-y-1.5">
                    <div className="flex justify-between text-sm font-medium">
                      ...
                      <span className="text-[#1a1a2e] font-mono font-bold">
        ...
        <div className="space-y-6">
          ...
          <div className="card p-6 rounded-2xl space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <span className="text-xs text-[#64748b] font-bold block mb-2.5">
                  Top Gainers
                </span>
                <div className="space-y-2">
                  {initialGainers.map(g => (
                    <div key={g.symbol} className="flex justify-between items-center text-xs font-medium">
                      <span className="text-[#1a1a2e] font-bold">{g.symbol}</span>
                      <span className="text-[#10b981] font-mono font-bold">+{g.change}%</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {initialLosers.map(l => (
                    <div key={l.symbol} className="flex justify-between items-center text-xs font-medium">
                      <span className="text-[#1a1a2e] font-bold">{l.symbol}</span>
                      <span className="text-[#ef4444] font-mono font-bold">{l.change}%</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2 text-xs text-center font-mono font-bold">
                <div className="bg-[#10b981]/15 border border-[#10b981]/30 p-2 rounded-xl text-[#10b981]">IT<br/>+1.45%</div>
                <div className="bg-[#10b981]/15 border border-[#10b981]/30 p-2 rounded-xl text-[#10b981]">BANK<br/>+0.82%</div>
                <div className="bg-[#ef4444]/15 border border-[#ef4444]/30 p-2 rounded-xl text-[#ef4444]">PHARMA<br/>-0.65%</div>
                <div className="bg-[#10b981]/15 border border-[#10b981]/30 p-2 rounded-xl text-[#10b981]">METAL<br/>+0.22%</div>
                <div className="bg-[#ef4444]/15 border border-[#ef4444]/30 p-2 rounded-xl text-[#ef4444]">AUTO<br/>-1.20%</div>
                <div className="bg-slate-100 border border-slate-200 p-2 rounded-xl text-[#64748b]">FIN<br/>0.00%</div>
              </div>
            </div>

            {/* Market Status and Option Chain KPI */}
            <div className="pt-4 border-t border-slate-100 space-y-2.5 text-xs font-medium">
