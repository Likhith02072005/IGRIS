...
export default function MissionControl() {
 ...
 className="flex items-center gap-1.5 px-4 py-2 bg-[#ef4444] hover:bg-[#ef4444]/90 text-[#1a1a2e] font-bold uppercase tracking-wider rounded-xl transition-all"
 >
 ...
 {[
 { label: 'Total Algorithmic Capital', val: `₹₹{capital.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, color: 'text-[#1a1a2e]' },
 { label: "Today's Net ROI PnL", val: `₹{totalTodayPnL >= 0 ? '+' : ''}₹₹{totalTodayPnL.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}`, color: totalTodayPnL >= 0 ? 'text-[#10b981]' : 'text-[#ef4444]' },
 { label: 'Max Peak Drawdown', val: maxDrawdownPct, color: isFreshMode ? 'text-[#1a1a2e]' : 'text-[#ef4444]' },
 { label: 'Active Execution Sockets', val: '3 Nodes Online', color: 'text-[#7c3aed]' },
 ].map(card => (
 <div key={card.label} className="card p-5 rounded-lg">
 <span className="text-[9px] font-bold text-[#64748b] uppercase tracking-widest block mb-1">{card.label}</span>
 ...
 <div className="card rounded-lg border border-white/30/80 overflow-hidden">
 <div className="p-5 border-b border-white/30 /65 flex justify-between items-center text-xs font-bold text-[#94a3b8]">
 ...
 <thead>
 <tr className="border-b border-white/30 /20 text-[9px] font-bold text-[#64748b] uppercase tracking-wider">
 ...
 <td className="p-4">
 <span className="text-[#1a1a2e] font-bold block">{s.name}</span>
 <span className="text-[10px] text-[#64748b] font-mono block mt-0.5">ID: {s.id}</span>
 </td>
 <td className="p-4">
 <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider ₹{
 isRunning ? 'bg-[#10b981]/10 text-[#10b981]' : isSuspended ? 'bg-[#ef4444]/10 text-[#ef4444]' : 'bg-gray-800 text-[#94a3b8]'
 }`}>
 {s.status}
 </span>
 </td>
 ...
 <td className="p-4 text-right font-mono text-[#94a3b8]">{isFreshMode ? '0.00%' : `₹{s.drawdown}%`}</td>
 <td className="p-4">
 ...
 <td className="p-4">
 <span className="text-[#10b981]">H:{s.health}%</span>
 <span className="text-[#64748b] mx-1">|</span>
 <span className="text-[#7c3aed]">AI:{s.aiScore}%</span>
 </td>
 ...
 className="p-2 rounded border border-white/30 text-[#94a3b8] hover:text-[#1a1a2e] transition-colors"
 ...
 className="p-2 rounded border border-white/30 text-[#10b981] hover:text-[#1a1a2e] transition-colors"
